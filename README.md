...
- **서브모니터 최적화** — 원하는 모니터에만 전체화면/키오스크로 고정, 해상도(FHD · QHD)에 맞춰 자동 정돈. 터치 디스플레이 지원.
...
- **인앱 업데이트 확인** — 설정 → 일반 탭에서 GitHub 최신 릴리즈를 확인. 새 버전이 있으면 릴리즈 노트와 다운로드 링크를 안내하고 기어(⚙)에 알림 점을 표시(서명 없는 빌드라 자동 설치 대신 알림·다운로드까지).
